...
export const usePlaceListLogic = () => {
  ...
  const toggleFavorite = (placeId) => {
    ...
    if (favorite) {
      axios
        .delete(`${apiUrl}/api/favorites/${favorite.id}`)
        .then(() => {
  ...
  const addPlaceToPlaylist = async (playlistId) => {
    ...
    try {
      ...
      const alreadyExists = playlist.places.some(
  ...
  const createPlaylistAndAddPlace = async () => {
    ...
    try {
      const response = await fetch(
        `${apiUrl}/api/playlists?userId=${userId}&name=${encodeURIComponent(
          newPlaylistName
        )}`,
        {
          method: "POST",
      ...
      const res = await fetch(
        `${apiUrl}/api/playlists/${playlist.id}/places/${selectedPlaceId}`,
        {
          method: "POST",
        }
      );
